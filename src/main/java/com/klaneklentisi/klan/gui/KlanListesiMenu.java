...
public class KlanListesiMenu extends Menu {
    ...
    public KlanListesiMenu(KlanEklentisi eklenti, Player oyuncu, int sayfa) {
    ...
    protected String baslik() {
        return mesajlar.baslik("menu.klan-listesi.baslik");
    }
    ...
    protected void doldur() {
        ...
        for (int i = baslangic; i < bitis; i++) {
            ...
            List<String> lore = mesajlar.alListe("menu.klan-listesi.satir-aciklama", Map.of(
                    "etiket", k.getEtiket(),
        ...
        if (sayfa > 0) {
            envanter.setItem(48, Esya.olustur(Material.ARROW, mesajlar.baslik("menu.klan-listesi.onceki-sayfa")));
        }
        if (bitis < klanlar.size()) {
            envanter.setItem(50, Esya.olustur(Material.ARROW, mesajlar.baslik("menu.klan-listesi.sonraki-sayfa")));
        }
        envanter.setItem(49, Esya.olustur(Material.BARRIER, mesajlar.baslik("menu.klan-listesi.geri")));
    }
    ...
    public void tikla(InventoryClickEvent olay) {
        ...
        if (slot == 48 && sayfa > 0) {
            new KlanListesiMenu(eklenti, oyuncu, sayfa - 1).ac();
            return;
        }
        ...
        if (slot < SAYFA_BASI && baslangic + slot < klanlar.size()) {
            ...
            new KlanBilgiMenu(eklenti, oyuncu, secilen).ac();
        }
    }
}
